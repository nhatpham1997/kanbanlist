...
const TaskItem = ({ task, provided }) => {
  ...
  const onClick = (taskId, task)=>{
    dispatch(taskActions.doSelectTask(taskId));
    dispatch(taskActions.listFile(taskId));
    dispatch(taskActions.listImage(taskId));
    dispatch(taskActions.listComment(task));
  }
  return (
      <div
          onClick={() => onClick(task.shortid, task._id)}
